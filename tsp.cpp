...
void Graph::addEdgeOrient(int src, int dest, int weight) // add a edge
{
    map_edges[make_pair(src, dest)] = weight;
...
map<pair<int, int>, int> Graph::showGraph() // shows all connections of the graph
{
...
vector<int> Genetic::run()
{
    ...
    //for(int i = 0; i < graph->V; i++)
    //    cout << vec[i] << " ";
    vec.push_back(graph->initial_vertex);
    vec.push_back(population[0].second);
    return vec;
    //cout << graph->initial_vertex;
